...
function findHack(arr) {
  ...
  return arr.filter(([name,score,grades]) => {
    //check if every grade is either A or B
    const isGradesAorB = grades.length > 4 && grades.every(val => (val === 'B') || (val === 'A'))
    const totalScore = grades.reduce((acc,c) => acc + (points[c] || 0),0)
    const finalscore = isGradesAorB ? totalScore + 20 : totalScore
