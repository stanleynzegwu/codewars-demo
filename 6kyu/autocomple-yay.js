...
function autocomplete(input, dictionary){
    let inp = input.toLowerCase().split('').filter(l => 
      (l.charCodeAt(0) >= 97) && (l.charCodeAt(0) <= 122)).join('')
    
    return dictionary.filter(word => word.toLowerCase().startsWith(inp)).splice(0,5)
  }